//!autogenerated 
//src: D:\workspaces\stack-2021\work-tests\jetgui-viewer-standalone_win\layout\jml\storage\storage.jml
//md5: a8c59e232855ebc0e9a3b77cafe27f4d
		...
		result.push({
			...
			e.addChild({
						...
						for(tab in iterable) {
							...
							if(condition) {
								e.addChild({
									...
									e.classList = SafeCode.run("navigation__divider");
									e;
								});
							}
							e.addChild({
								...
								e.classList = SafeCode.run( tab.isSelected ? "navigation__link active" : "navigation__link");
								e;
							});
						}
					}
				...
				e.addChild({
					...
					e.height = SafeCode.run(64);
					e.width = SafeCode.run(320);
					e.fillColor = SafeCode.run("333333");
					e.addChild({
						var e = new VDomElement(VDomIdMacro.makeId());
						e.styleRules = styleRules;
						e.tag = SafeCode.run("box");
						e.offsetY = SafeCode.run(0);
						e.fillColor = SafeCode.run(green);

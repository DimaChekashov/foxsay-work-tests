//!autogenerated 
//src: D:\workspaces\stack-2021\foxsay-work-tests\zavod-games\layout\jml\storage\prefabStorageItem.jml
//md5: 6b42fc323b24e60f54bab28df68d1267
...
class StoragePrefabstorageitemComponent extends Component {
	...
	override public function render(vDom: VDomComponent): Null<Array<VDomNode>> {
		...
		result.push({
			...
			e.classList = SafeCode.run( storageItem.modeSale ? "granary-item sale-mode" : "granary-item");
			e.addChild({
				var e = new VDomElement(VDomIdMacro.makeId());
				e.styleRules = styleRules;
				e.tag = SafeCode.run("box");
				e.classList = SafeCode.run("granary-item__header");
				...
				if(condition) {
					e.addChild({
						...
						e.styleRules = styleRules;
						e.tag = SafeCode.run("box");
						e.classList = SafeCode.run("granary-item__zoom-btn");
						e;
					});
				}
				e.addChild({
					...
					e.classList = SafeCode.run( storageItem.modeSale ? "granary-item__img sale-mode" : "granary-item__img");
					e;
				});
				e;
			});
			...
			if(condition) {
				e.addChild({
					...
					e.tag = SafeCode.run("text");
					e.value = SafeCode.run(storageItem.count);
					e.classList = SafeCode.run("granary-item__count sale-mode");
					e;
				});
				e.addChild({
					var e = new VDomElement(VDomIdMacro.makeId());
					e.styleRules = styleRules;
					e.tag = SafeCode.run("vbox");
					e.classList = SafeCode.run("sale-mode__block");
					e.addChild({
						var e = new VDomElement(VDomIdMacro.makeId());
						e.styleRules = styleRules;
						e.tag = SafeCode.run("box");
						e.classList = SafeCode.run("sale-mode__counter");
						e.addChild({
							var e = new VDomElement(VDomIdMacro.makeId());
							e.styleRules = styleRules;
							e.tag = SafeCode.run("box");
							e.classList = SafeCode.run("sale-mode__minus");
							e;
						});
						e.addChild({
							var e = new VDomElement(VDomIdMacro.makeId());
							...
							e.classList = SafeCode.run("sale-mode__count");
							e;
						});
						e.addChild({
							var e = new VDomElement(VDomIdMacro.makeId());
							e.styleRules = styleRules;
							e.tag = SafeCode.run("box");
							e.classList = SafeCode.run("sale-mode__plus");
							e;
						});
						e;
					});
					e.addChild({
						...
						e.classList = SafeCode.run("sale-mode__price");
						e.addChild({
							var e = new VDomElement(VDomIdMacro.makeId());
							e.styleRules = styleRules;
							e.tag = SafeCode.run("img");
						...
						e.addChild({
							...
							e.classList = SafeCode.run("sale-mode__price__text");
							e;
						});
						e;
					});
					e.addChild({
						...
						e.styleRules = styleRules;
						e.tag = SafeCode.run("text");
						e.value = SafeCode.run("Sell");
						e.classList = SafeCode.run("sale-mode__btn");
						e;
					});
					e;
				});
